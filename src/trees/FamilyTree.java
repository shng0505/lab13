...
public class FamilyTree {
	...
	private void addLine(String line) throws TreeException {
		...
		if (colonIndex < 0) {
			throw new TreeException("Invalid");
		}
		String parent = line.substring(0, colonIndex); // The substring of line that starts at char #0 and ends just
														// before colonIndex. Check the API for
		// class java.util.String, method substring(), if you need guidance.
		String childrenString = line.substring(colonIndex + 1); // The substring of line that starts just after
																// colonIndex and goes through the end of
		// the line. You'll use a different version of substring().
		String[] childrenArray = childrenString.split(","); // Call childrenString.split(). Check the API for details.
