...
public class FamilyTree
{
	...
	private void addLine(String line) throws TreeException
	{
		...
		if (colonIndex < 0)
		{
			throw new TreeException("colonIndex is negative"); //throw a TreeException with a useful message
		}
		...
		else
		{
			...
			if(parentNode == null) //If the parent node wasn't found, there must have been something wrong in the 
				throw new TreeException("Parent Node not found"); //data file. Throw an exception.
		}
		
		// Add child nodes to parentNode.
		for(String childName : childrenArray) { //For each name in childrenArray, create a new node and add that node to parentNode.
	...
	TreeNode getMostRecentCommonAncestor(String name1, String name2) throws TreeException
	{
		...
		if (node1 == null)
			throw new TreeException("node1 is null"); //Throw a TreeException with a useful message
		TreeNode node2 = root.getNodeWithName(name2);		// node whose name is name2
		if (node2 == null)
			throw new TreeException("node2 is null"); //Throw TreeException with a useful message
